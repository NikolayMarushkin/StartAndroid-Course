package com.course.p0361sqlitequery;
...
public class MainActivity extends Activity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            // Все записи
            case R.id.btnAll:
                Toast.makeText(getApplicationContext(), "--- Все записи ---", Toast.LENGTH_SHORT).show();
                c = db.query("mytable", null, null, null, null, null, null);
                break;
            // Функция
            case R.id.btnFunc:
                Toast.makeText(getApplicationContext(), "--- Функция " + sFunc + " ---", Toast.LENGTH_SHORT).show();
                columns = new String[] { sFunc };
                c = db.query("mytable", columns, null, null, null, null, null);
                break;
            // Население больше, чем
            case R.id.btnPeople:
                Toast.makeText(getApplicationContext(), "--- Население больше " + sPeople + " ---", Toast.LENGTH_SHORT).show();
                selection = "people > ?";
            ...
            case R.id.btnGroup:
                Toast.makeText(getApplicationContext(), "--- Население по региону ---", Toast.LENGTH_SHORT).show();
            ...
            case R.id.btnHaving:
                Toast.makeText(getApplicationContext(), "--- Регионы с населением больше " + sRegionPeople + " ---", Toast.LENGTH_SHORT).show();
                columns = new String[] { "region", "sum(people) as people" };
                groupBy = "region";
            ...
            case R.id.btnSort:
                ...
                switch (rgSort.getCheckedRadioButtonId()) {
                    // наименование
                    case R.id.rName:
                        Toast.makeText(getApplicationContext(), "--- Сортировка по наименованию ---", Toast.LENGTH_SHORT).show();
                        orderBy = "name";
                        break;
                    // население
                    case R.id.rPeople:
                        Toast.makeText(getApplicationContext(), "--- Сортировка по населению ---", Toast.LENGTH_SHORT).show();
                        orderBy = "people";
                        break;
                    // регион
                    case R.id.rRegion:
                        Toast.makeText(getApplicationContext(), "--- Сортировка по региону ---", Toast.LENGTH_SHORT).show();
                        orderBy = "region";
                        break;
                }
